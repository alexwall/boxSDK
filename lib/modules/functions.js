...
function uploadRequest(callb,path,file,headers){
	var nameFilePath = file.filename.split('/');
	var nameDefault = nameFilePath[nameFilePath.length - 1];
	
<<<<<<< HEAD
	var name = file.customName || nameDefault;
=======
	var name = file.customName || nameFilePath;
>>>>>>> f8e88a8089c7a8554a7e0ad1dd9511f0cda327a5
...
getHeaders = function (token,type,content){
	...
	if (typeof content !== 'undefined' && typeof content.user === 'string' && content.user.length > 0) {
		result['As-User'] = content.user;
	}
	return result
}
